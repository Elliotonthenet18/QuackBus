...
class DownloadService {
  ...
  async downloadTrack(trackId, quality = 7, broadcastFn) {
    ...
    try {
      console.log(`📥 Creating download job for track ID: ${trackId}, quality: ${quality}`);
      
      // Create download info with just the track ID
      // We'll get the download URL directly without needing track details
      const downloadInfo = {
        id: downloadId,
        type: 'track',
        trackId,
      ...
      console.log(`✅ Track download ${downloadId} queued for track ${trackId}`);
      this.processQueue();
  ...
  async processTrackDownload(downloadInfo) {
    const { trackId, quality, broadcastFn } = downloadInfo;
    
    try {
      console.log(`🎵 Processing download for track ID: ${trackId}`);
      
      downloadInfo.status = 'downloading';
      this.broadcast(downloadInfo, broadcastFn);
      
      // Get download URL directly using track ID from search/album results
      console.log(`🌐 Requesting download URL: https://qobuz-proxy.authme.workers.dev/api/download-music?track_id=${trackId}&quality=${quality}`);
      
      const fileUrlData = await qobuzService.getTrackFileUrl(trackId, quality);
      console.log(`📊 Received response:`, fileUrlData);
      ...
      console.log(`✅ Got download URL: ${fileUrlData.url}`);
      
      // Prepare file paths - use track ID as filename for now
      const qualityInfo = qobuzService.getQualityInfo(quality);
      const fileName = `${trackId}.${qualityInfo.extension}`;
      
      const finalDir = this.downloadPath;
      ...
      console.log(`📥 Starting file download to: ${tempFile}`);
      ...
      await fs.move(tempFile, finalFile);
      console.log(`✅ File saved to: ${finalFile}`);
